...
describe('GenerationApiClient', () => {
  it('maps a malformed server payload without exposing it', async () => {
    ...
    await expect(new GenerationApiClient('http://localhost', async () => 'test-session-token').generate(request, new AbortController().signal))
  ...
  it('returns a validated no-match response', async () => {
    ...
    const result = await new GenerationApiClient('http://localhost', async () => 'test-session-token').generate(request, new AbortController().signal);
  ...
  it('reads persisted recipe history and records a visit through the remote API', async () => {
    ...
    const client = new GenerationApiClient('http://localhost', async () => 'test-session-token');
    await expect(client.listHistory('zh-CN', new AbortController().signal)).resolves.toMatchObject({ items: [{ recipe: { recipeId: RECIPE_FIXTURES[0].recipeId } }] });
    await expect(client.recordHistoryVisit({ recipeId: RECIPE_FIXTURES[0].recipeId, source: 'remote' }, new AbortController().signal)).resolves.toBeUndefined();
