...
export default function HistoryScreen() {
  const { t, i18n } = useTranslation(); const { state, cacheRecipe, addRecentRecipe } = useP0Store();
  ...
  useEffect(() => { if (!repository) return; let active = true; const controller = new AbortController(); void repository.listHistory(state.guestId, locale, controller.signal).then((history) => { if (!active) return; history.items.forEach((entry) => { cacheRecipe(entry.recipe); addRecentRecipe({ recipeId: entry.recipe.recipeId, viewedAt: entry.lastVisitedAt, source: entry.source, locale: entry.recipe.locale }); }); setError(null); }).catch(() => { if (active) setError(t('history.refreshFailed')); }).finally(() => { if (active) setHasLoaded(true); }); return () => { active = false; controller.abort(); }; }, [addRecentRecipe, attempt, cacheRecipe, locale, repository, state.guestId, t]);
