...
export default function HomeScreen() {
  ...
  return <Screen>
    ...
    <AppCard><ThemedText type="sectionTitle">{t('home.ingredients')}</ThemedText><ThemedText type="small" themeColor="textSecondary">{t('home.selectionHint')}</ThemedText><IngredientCategoryTabs selected={state.uiPreferences.selectedCategory} onChange={setSelectedCategory} /><IngredientSearch value={query} onChange={setQuery} /><IngredientGrid ingredients={ingredients} selectedIds={state.selectedIngredients.map((item) => item.id)} onToggle={toggleCatalogIngredient} /></AppCard>
