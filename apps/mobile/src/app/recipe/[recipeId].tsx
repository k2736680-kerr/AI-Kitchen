...
export default function RecipeDetailScreen() {
  ...
  const source = existingEntry?.source ?? (state.generation.recipeId === recipeId
    ...
    : 'fixture');
  useEffect(() => {
    if (recipe) addRecentRecipe({ recipeId: recipe.recipeId, viewedAt: new Date().toISOString(), source: existingEntry?.source ?? source });
  }, [addRecentRecipe, existingEntry?.source, recipe, source]);
