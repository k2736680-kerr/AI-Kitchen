...
describe('MySqlRecipePersistence', () => {
  it('rolls back the transaction when recipe persistence fails', async () => {
    ...
    const response: Extract<GenerationApiResponse, { status: 'success' }> = {
      status: 'success', schemaVersion: 'v1', requestId: request.requestId, recipes: [RECIPE_FIXTURES[0]],
    ...
    await expect(persistence.saveRecipeSuccess({ request, requestHash: 'hash', response, recipes: [RECIPE_FIXTURES[0]], durationMs: 1 })).rejects.toThrow('write failed');
