...
export async function createApiApp(dependencies: ApiDependencies): Promise<FastifyInstance> {
  ...
  app.post('/api/v1/recipes/generate', async (request, reply) => {
    ...
    const rateKey = input.identity.type === 'guest' ? input.identity.guestId : input.identity.userId;
    ...
    try {
      const response = await generation.generate(input);
      request.log.info({ requestId: input.requestId, status: response.status }, 'recipe_generation_completed');
  ...
  app.get('/api/v1/recipes/:recipeId', async (request, reply) => {
    ...
    const recipe = await dependencies.persistence.getRecipe(recipeId);
  ...
  app.post('/api/v1/history/visit', async (request, reply) => {
    ...
    const recorded = await dependencies.persistence.visitHistory(visit.data);
