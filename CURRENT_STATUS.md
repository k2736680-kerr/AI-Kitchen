...
## P0 体验优化：烹饪计时器与空状态引导（2026-08-05，IMPLEMENTED·待上机）

- **烹饪计时器**（`features/cooking/cooking-step-card.tsx`）：步骤含时长时渲染可点击倒计时，补齐竞品标配的逐步引导计时能力（下厨房 / 美食杰 / SideChef 均有）。
- **历史空状态引导**：新增 `components/empty-state.tsx` 通用组件；历史页无记录时给出引导 + 「去生成菜谱」入口，结束此前「空列表无提示」的体验缺口。
- 移动端 `tsc --noEmit` 通过（EXIT=0）。
- **阻塞未解**：Android 原生 APK 仍因 Windows 构建环境受限未出包，计时器 / 空状态暂以 Web 预览 + 类型检查验证；需用户用本机 Android Studio 打开 `apps/mobile/android` 并 Run（选 Pixel_8a / emulator-5554），或配置 EAS 云构建后上机实测。
- 同日已先将 2026-08-04 的 82 项未提交改动存为存档点（commit `c0851bbe`），规避丢失风险。
