...
export class AliyunQwenRecipeProvider implements RecipeProvider {
  ...
  public constructor(private readonly config: DashscopeProviderConfig) { this.model = config.model; }
  ...
  public async generateBatch(request: GenerationRequest, signal: AbortSignal): Promise<ReadonlyArray<unknown | null>> {
    ...
    const attempts = await Promise.allSettled(methods.map((method) => this.call(buildRecipeSystemPrompt(request.locale), buildRecipeUserPrompt(request, method), signal)));
    ...
    try {
      ...
      try {
        response = await fetch(`${this.config.baseUrl}/chat/completions`, {
          method: 'POST', headers: { Authorization: `Bearer ${this.config.apiKey}`, 'Content-Type': 'application/json', Accept: 'application/json' },
          body: JSON.stringify({ model: this.config.model, temperature: this.config.temperature, top_p: this.config.topP, stream: false, response_format: { type: 'json_object' }, enable_thinking: false, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }] }),
...
function pickCookingMethods(count: number, excludedRecipes: ReadonlyArray<{ readonly title: string; readonly cookingMethod: (typeof COOKING_METHOD_OPTIONS)[number] }> = []): readonly (typeof COOKING_METHOD_OPTIONS)[number][] {
